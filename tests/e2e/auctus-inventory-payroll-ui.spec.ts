...
function requireEnv(value: string | undefined, name: string): string {
  if (!value) throw new Error(`${name} is required for inventory/payroll UI smoke tests.`);
  return value;
}
...
async function createProduct(page: Page, name: string, sku: string) {
  ...
  await sheet.getByLabel('Unit of Measure').fill('unit');
  await sheet.getByLabel('Cost Price').fill('12');
  await sheet.getByLabel('Sell Price').fill('30');
  await sheet.getByLabel('Reorder Point').fill('3');
...
async function receivePurchaseOrder(page: Page, productName: string, supplierName: string) {
  ...
  await createSheet.getByLabel('Memo').fill('Inventory payroll UI smoke');
...
async function createSaleMovement(page: Page, productName: string) {
  ...
  await sheet.getByLabel('Memo').fill('UI smoke sale movement');
  await waitForWrite(page, '/inventory-movements', async () => {
    await sheet.getByRole('button', { name: 'Save' }).click();
  });

  await page.getByRole('button', { name: 'Movements' }).click();
  await expect(page.getByText('UI smoke sale movement')).toBeVisible();
...
async function createEmployee(page: Page, name: string) {
  ...
  await sheet.getByLabel('Super Fund').fill('UI Smoke Super');
...
async function recordPaygRemittance(page: Page) {
  ...
  await expect(paygCard.getByRole('button', { name: 'Record Remittance' })).toBeVisible();
  await paygCard.getByRole('button', { name: 'Record Remittance' }).click();

  const sheet = modal(page, 'Record Remittance');
  await sheet.getByLabel('Memo').fill('UI smoke PAYG remittance');
  await waitForWrite(page, '/remittances', async () => {
    await sheet.getByRole('button', { name: 'Save' }).click();
  });

  await expect(page.getByText('UI smoke PAYG remittance')).toBeVisible();
  await expect(page.getByText('PAYG', { exact: true })).toBeVisible();
}
...
test.describe('Inventory and payroll cloud UI flows', () => {
  ...
  test('persists inventory and payroll records through the web UI', async ({ page }) => {
    page.on('dialog', (dialog) => dialog.accept());

    const admin = adminClient();
    const runId = Date.now();
    const workspaceName = `Inventory Payroll UI ${runId}`;
    const productName = `UI Widget ${runId}`;
    const productSku = `UIW-${runId}`;
    const supplierName = `UI Supplier ${runId}`;
    const employeeName = `UI Employee ${runId}`;
    const stpReference = `STP-UI-${runId}`;
