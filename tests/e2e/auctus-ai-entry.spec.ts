...
test.describe('Auctus AI quick entry', () => {
          ...
          content: [
            {
              ...
              input: {
                ...
                amount: 49,
                date: '2026-06-19',
                accountId: 'a2 | Everyday Account (bank)',
  ...
  test('normalizes signed strict amount strings from local AI output', async ({ page }) => {
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          content: [
            {
              type: 'tool_use',
              id: 'toolu_amount_string_ai_entry',
              name: 'parse_transaction',
              input: {
                type: 'expense',
                amount: '(AUD 1,234.50)',
    ...
    const draft = page.locator('.ai-entry-draft');
    await expect(draft).toContainText('$1234.50');
  ...
  test('normalizes common date formats from local AI output', async ({ page }) => {
    ...
    await page.getByTitle('AI Quick Entry').click();
    await page.locator('.ai-entry-textarea').fill('Spent $88 on 2026-6-20');
    await page.getByRole('button', { name: /Parse/i }).click();
  ...
  test('normalizes natural language invoice payment terms', async ({ page }) => {
    await page.route('https://api.anthropic.com/v1/messages', async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({
          content: [
            {
              type: 'tool_use',
              id: 'toolu_invoice_terms_text_ai_entry',
              name: 'parse_transaction',
              input: {
                type: 'income',
                amount: 640,
    ...
    await page.getByTitle('AI Quick Entry').click();
    await page.locator('.ai-entry-textarea').fill('Invoice $640 Net 30 on 2026-06-10');
    await page.getByRole('button', { name: /Parse/i }).click();
    ...
    const modal = page.locator('.sheet').filter({ hasText: 'New Transaction' });
    await expect(modal).toBeVisible();
    await expect(modal.getByLabel('Terms')).toHaveValue('net_30');
    await expect(modal.getByRole('textbox', { name: 'Due Date' })).toHaveValue('2026-07-10');
  });
