...
import { allPayrollJournalEntries, allRemittanceJournalEntries } from './payroll.js';
...
export function allJournalEntries(data: LedgerData): JournalEntry[] {
  ...
  for (const entry of allPayrollJournalEntries(data)) {
    entries.push(entry);
  }
  for (const entry of allRemittanceJournalEntries(data)) {
    entries.push(entry);
  }
